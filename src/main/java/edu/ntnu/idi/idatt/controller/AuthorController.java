...
/**
 * Controller for author-related actions.
 * Handles viewing and managing authors.
 */
public class AuthorController {
    ...
    public Action showAuthorMenu(Scanner in, PrintStream out) {
        ...
        while (true) {
            ...
            switch (choice) {
                case "1" -> { return this::showAuthorsList; }
                case "2" -> { return this::showCreateAuthor; }
                case "3" -> { return this::showFindAuthor; }
                case "b" -> { return (in2, out2) -> mainMenuController.showMenu(in2, out2); }
                default -> {
    ...
    public Action showCreateAuthor(Scanner in, PrintStream out) {
        ...
        if (firstName.isBlank()) return this::showAuthorMenu;

        createAuthorView.promptLastName(out);
        String lastName = in.nextLine().trim();
        if (lastName.isBlank()) return this::showAuthorMenu;
        ...
        while (true) {
            ...
            if (email.isBlank()) return this::showAuthorMenu;
     ...
     * @return the next action to execute
     */
    public Action showAuthorDetail(Author author, Action backDestination, Scanner in, PrintStream out) {
        ...
        while (true) {
            ...
            switch (choice) {
                case "1" -> { return (in2, out2) -> viewAuthorEntries(author, backDestination, in2, out2); }
                case "2" -> { return (in2, out2) -> editAuthor(author, backDestination, in2, out2); }
                case "3" -> { return (in2, out2) -> deleteAuthor(author, backDestination, in2, out2); }
                case "b" -> { return backDestination; }
                default -> {
     ...
     */
    private Action viewAuthorEntries(Author author, Action backDestination, Scanner in, PrintStream out) {
